...
const CategoryPopup: React.FC<CategoryProps> = ({ onClose, categoryData , fetchData }) => {
    ...
    const UpdateCategory = async() => {
        ...
        try { 
            const response = await axios.put(`${baseUrl}/categories/update/${category._id}`, data,{
                headers: {
                    token: `Bearer ${Token}`,
                  }
            });
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="w-full p-6 text-black lg:max-[90vh] md:max-h-[85vh] max-h-[90vh] overflow-y-auto bg-white rounded-lg shadow-lg lg:w-2/3">
                <h2 className="mb-4 text-lg font-bold text-center text-black">Edit Category</h2>
