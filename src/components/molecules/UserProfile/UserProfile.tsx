...
const UserProfile:React.FC = () => {
  ...
  const handleSubmitCreate = () => {
    if(!newRule.deviceName || !newRule.emailStatus){
      notify('Choose device and email status before click save button!' , 'warning');
      return
    }
    Swal.fire({
      title: "",
  ...
  const handleUpdateFunction = () => {
    if(UserUpdateData.currentPassword && UserUpdateData.confirmPassword && UserUpdateData.newPassword){
    ...
    if(!UserUpdateData.currentPassword && !UserUpdateData.newPassword && !UserUpdateData.confirmPassword){
      Swal.fire({
        title: "",
  ...
  const UpdateUserDetails = async() => {
    ...
    try {
      const response = await axios.put(
        `${baseUrl}/users/update/${UserUpdateData.id}`, data , {
            headers: {
            token: `Bearer ${Token}`,
          }
        }
  ...
  const UpdateAuthenticationDetails = async() => {
    ...
    try {
      const response = await axios.put(
        `${baseUrl}/users/update/auth/${UserUpdateData.id}`, data , {
            headers: {
            token: `Bearer ${Token}`,
          }
        }
  ...
  return (
    ...
    <div className={`max-w-6xl p-8 mx-auto mt-8 rounded-lg shadow-lg ${theme === 'light' ? "bg-gray-300" : "bg-white" }`}>
      ...
      <div className="flex flex-col items-center justify-start lg:space-x-8 lg:items-start lg:flex-row">
        {/* Profile Image */}
        <div className="w-32 h-32 overflow-hidden border-2 border-gray-200 rounded-full shadow-md cursor-pointer" onClick={togglePopup}>
          {UserData?.imageUrl ? (
            <img src={UserData.imageUrl} alt="Profile" className="object-cover w-full h-full" />
          ) : (
            <img src={Images.unknownUser} alt="Profile" className="object-cover w-full h-full" />
          )}
        </div>
        ...
        <div className='cursor-default'>
          ...
          <p className="mb-2 text-center text-gray-600 lg:mt-0 lg:text-start">{UserData?.organization}</p>

          {/* User Status */}
          <span
            className={`mt-2 inline-block px-4 py-3 lg:w-fit w-full rounded-md text-sm font-medium text-center lg:mt-0 lg:text-start 
              ${
              UserData?.status === "Active" ? 'bg-green-300 text-green-700' : 'bg-red-300 text-red-700'
            }`}
          >
      ...
      <div className="grid grid-cols-1 gap-8 mt-7 md:grid-cols-2">
        <div className="p-5 bg-gray-200 rounded-lg shadow-sm">
          <h3 className="font-semibold text-center text-gray-700 md:text-left">User Type</h3>
          <p className="mt-2 text-center text-gray-900 md:text-left">{UserData?.userType}</p>
        </div>
        <div className="p-5 bg-gray-200 rounded-lg shadow-sm">
          <h3 className="font-semibold text-center text-gray-700 md:text-left">Created At</h3>
          <p className="mt-2 text-center text-gray-900 md:text-left">{UserData?.dateCreated} at {UserData?.timeCreated}</p>
        </div>
        <div className="p-5 bg-gray-200 rounded-lg shadow-sm">
          <h3 className="font-semibold text-center text-gray-700 md:text-left">Updated At</h3>
          <p className="mt-2 text-center text-gray-900 md:text-left">{UserData?.dateUpdated} at {UserData?.timeUpdated}</p>
        </div>
      ...
      {showEditForm && (
        ...
        <div className="w-full p-6 text-black lg:max-[90vh] md:max-h-[85vh] max-h-[90vh] overflow-y-auto bg-white rounded-lg shadow-lg lg:w-2/3">
          <h2 className="mb-4 text-lg font-bold text-center text-black">Edit Profile</h2>
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
            <div className=''>
            <label htmlFor="fullName" className="w-full font-semibold text-[13px]">Full Name</label>
              <input
                type="text"
                name="fullName"
                placeholder='Full Name'
                value={UserUpdateData.fullName}
                onChange={(e) => setUserUpdateData({...UserUpdateData, fullName:e.target.value})}
                className="w-full p-2 mt-2 text-[12px] border rounded-md"
              />
            </div>
            <div className=''>
            <label htmlFor="image" className="w-full font-semibold text-[13px]">Image</label>
              <input
                type="file"
                name="image"
            ...
            <label htmlFor="emailAddress" className="w-full font-semibold text-[13px]">Email Address</label>
              <input
                type="email"
                name="emailAddress"
                placeholder='Email Address'
                value={UserUpdateData.emailAddress}
                onChange={(e) => setUserUpdateData({...UserUpdateData , emailAddress:e.target.value})}
                className="w-full p-2 mt-2 text-[12px] border rounded-md"
              />
            </div>
            <div>
            <label htmlFor="phoneNumber" className="w-full font-semibold text-[13px]">Phone Number</label>
              <input
                type="tel"
                name="phoneNumber"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setUserUpdateData({...UserUpdateData , organization:e.target.value})}
                className="w-full p-2 mt-2 text-[12px] border rounded-md"
              />
            </div>
            <div className='md:col-span-2'>
              <label htmlFor="address" className="w-full font-semibold text-[13px]">Address</label>
              <textarea
                name="address"
                placeholder='Address'
                value={UserUpdateData.address}
                onChange={(e) => setUserUpdateData({...UserUpdateData , address:e.target.value})}
                className="w-full p-2 mt-2 text-[12px] border rounded-md"
              />
            </div>
            {/* User Type Dropdown */}
            {UserType === "Admin"  ? (
              <>
                <div>
                  <label className="w-full font-semibold text-[13px]">User Type</label>
                  <select
                    name="userType"
                ...
                {/* Status Dropdown */}
                <div>
                  <label className="w-full font-semibold text-[13px]">Status</label>
                  <select
                    name="status"
                    value={UserUpdateData.status}
                    onChange={(e) => setUserUpdateData({...UserUpdateData , status:e.target.value})}
                    className="w-full p-2 mt-2 text-[12px] border rounded-md"
                  >
            ...
            {UserId === userId ? (
              ...
              <div></div>
                <div>
              <label className="w-full font-semibold text-[13px]">Current Password</label>
              <input
                type="password"
                name="oldPassword"
                placeholder='Current Password'
                onChange={(e) => setUserUpdateData({...UserUpdateData , currentPassword:e.target.value })}
                className="w-full p-2 mt-2 text-[12px] border rounded-md"
              />
            </div>
            <div>
              <label className="w-full font-semibold text-[13px]">New Password</label>
              <input
                type="password"
                name="newPassword"
                placeholder='New Password'
                onChange={(e) => setUserUpdateData({...UserUpdateData , newPassword:e.target.value})}
                className="w-full p-2 mt-2 text-[12px] border rounded-md"
              />
            </div>
            <div>
              <label className="w-full font-semibold text-[13px]">Confirm Password</label>
              <input
                type="password"
                name="confirmPassword"
                onChange={(e) => setUserUpdateData({...UserUpdateData , confirmPassword:e.target.value})}
    ...
    {isFormOpen && (
      ...
      <div className="w-full p-6 bg-white rounded-lg shadow-lg lg:w-2/3 sm:w-[95%]">
        <h2 className="mb-4 text-lg font-bold text-center text-black">Create New Rule</h2>
